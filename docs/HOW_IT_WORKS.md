...
4. **Hydration & Persistence Guarding:** In the resume module, caching behaviors in the Next.js App Router caused hydration mismatches where initial server state wasn't synced with the database. This was resolved using `export const dynamic = "force-dynamic"` to ensure fresh queries.
5. **Direct PDF Streaming (No S3):** Rather than generating PDFs locally and uploading them to S3, the report generation pipeline constructs PDFs entirely in memory using `fpdf2` and streams the binary data directly back to the client via `StreamingResponse`. This stateless design improves generation speed and removes complex cloud storage dependencies.
6. **Backend-For-Frontend (BFF) Auth Proxy:** The system uses `httpOnly` cookies for token storage to prevent XSS attacks. Because browser clients cannot read these cookies, a Next.js API route acts as a proxy. It reads the secure cookie server-side and attaches it as a `Bearer` token to the outgoing backend request, ensuring secure communication without exposing tokens to the frontend.
